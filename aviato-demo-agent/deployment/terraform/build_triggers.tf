...
resource "google_cloudbuild_trigger" "pr_checks" {
  ...
  depends_on = [
    ...
    # resource.google_project_service.deploy_project_services, # This resource was removed
...
resource "google_cloudbuild_trigger" "cd_pipeline" {
  ...
  include_build_logs = "INCLUDE_BUILD_LOGS_WITH_STATUS"
  substitutions = {
    _DEPLOY_PROJECT_ID             = var.project_id # Consolidated project ID
    _BUCKET_NAME_LOAD_TEST_RESULTS = resource.google_storage_bucket.bucket_load_test_results.name # Still relevant
    _LOGS_BUCKET_NAME              = resource.google_storage_bucket.logs_data_bucket.url # Direct reference to single bucket
    _REGION                        = var.region
